...
const OCR_CODE = [
  ...
  'let buf = null;',
  'const binKey = json.binario_key || "data";',
  'const bin = $binary && $binary[binKey];',
  'if (bin) {',
  '  try { buf = await this.helpers.getBinaryDataBuffer(0, binKey); } catch(e) {}',
  '}',
  'if (!buf && item.binary) {',
  '  const ibk = Object.keys(item.binary);',
  '  if (ibk.length > 0) {',
  '    try { buf = await this.helpers.getBinaryDataBuffer(0, ibk[0]); } catch(e) {}',
  '  }',
  '}',
  'if (!buf) {',
  '  return [{ json: { ...json, ocr_status: "erro", ocr_erro: "sem_dados_binario" } }];',
  '}',
  'const mime = json.arquivo_tipo || "application/octet-stream";',
  'const dataUri = "data:" + mime + ";base64," + buf.toString("base64");',
  'const arquivoNome = json.arquivo_nome || "arquivo";',
  'let ocrStatus = "erro";',
  'let ocrTexto = "";',
...
const SET_META = [
  ...
  { id: "2", name: "arquivo_salvo", value: "={{ $json.ocr_path_txt || $json.caminho_completo || \"\" }}", type: "string" },
...
async function main() {
  ...
  try { execSync('docker compose up -d ocr-service', { stdio: 'pipe', timeout: 30000, shell: true }); } catch(e) {}
  ...
  } catch (e) { console.log('Health check: failed (continuing)'); }

  // === DEPLOY WORKFLOW ===
  console.log('\n=== Deploy Workflow ===');
  const wf = await getWF('WFCRM001comp01');
  const backup = 'backup_test_' + Date.now() + '.json';
  fs.writeFileSync(backup, JSON.stringify({ nodes: wf.nodes, connections: wf.connections }, null, 2));
  console.log('Backup saved.');

  // Fix VALIDAR ARQUIVO: remove $input.all()
  const validNode = wf.nodes.find(n => n.name === 'VALIDAR ARQUIVO');
  if (validNode) {
    let code = validNode.parameters.jsCode;
    code = code.replace('const items = $input.all();', '');
    code = code.replace(/const items = \$input\.all\(\);\n?/, '');
    code = code.replace(/^\s*\n/, '');
    validNode.parameters.jsCode = code;
  }

  // Keep only original nodes + add new ones
  ...
  const basePn = nodes.find(n => n.name === 'Write Binary File').position;
  ...
  const respAssigns = [{ id: '1', name: 'status', value: 'success', type: 'string' }, { id: '2', name: 'arquivo_original', value: '={{ $json.arquivo_nome || "" }}', type: 'string' }, { id: '3', name: 'arquivo_salvo', value: '={{ $json.ocr_path_txt || $json.caminho_completo || "" }}', type: 'string' }, { id: '4', name: 'timestamp', value: '={{ Date.now() }}', type: 'number' }];
  ...
  const dr = await api('PUT', 'workflows/WFCRM001comp01', JSON.stringify({ name: wf.name, nodes, connections: newConn, settings: wf.settings, staticData: wf.staticData || null }));
  ...
  function sendJson(data) {
    return new Promise(resolve => {
      ...
      const opts = { hostname: 'localhost', port: 5678, path: '/webhook/comprovante', method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': cl }, timeout: 60000 };
  ...
  // Test 1: PDF — workflow should accept and process (OCR may fail for PDF with Tesseract)
  console.log('--- Test 1: PDF ---');
  ...
  console.log('  status=' + r1.s + ' ocr_status=' + (j1.ocr_status || '?') + ' ocr_engine=' + (j1.ocr_engine || '?'));
  check('PDF', r1.s === 200 && j1.status === 'success' && j1.ocr_status !== undefined, 'ocr=' + (j1.ocr_status || '?') + ' eng=' + (j1.ocr_engine || '?'));

  // Test 2: JPG — workflow processes, OCR may fail with synthetic test image
  console.log('\n--- Test 2: JPG ---');
  ...
  console.log('  status=' + r2.s + ' ocr_status=' + (j2.ocr_status || '?') + ' ocr_engine=' + (j2.ocr_engine || '?'));
  check('JPG', r2.s === 200 && j2.status === 'success' && j2.ocr_status !== undefined, 'ocr=' + (j2.ocr_status || '?') + ' eng=' + (j2.ocr_engine || '?'));

  // Test 3: PNG — workflow processes, OCR may fail with synthetic test image
  console.log('\n--- Test 3: PNG ---');
  ...
  console.log('  status=' + r3.s + ' ocr_status=' + (j3.ocr_status || '?') + ' ocr_engine=' + (j3.ocr_engine || '?'));
  check('PNG', r3.s === 200 && j3.status === 'success' && j3.ocr_status !== undefined, 'ocr=' + (j3.ocr_status || '?') + ' eng=' + (j3.ocr_engine || '?'));

  // Test 4: OCR indisponivel — service down, should return error gracefully
  ...
  console.log('  status=' + r4.s + ' ocr_status=' + (j4.ocr_status || '?') + ' ocr_erro=' + (j4.ocr_erro || '?'));
  check('OCR indisponivel', r4.s === 200 && j4.status === 'success' && (j4.ocr_erro || '').includes('erro_conexao'), 'erro=' + (j4.ocr_erro || '?'));
  ...
  console.log('  status=' + r5.s + ' ocr_status=' + (j5.ocr_status || '?') + ' ocr_erro=' + (j5.ocr_erro || '?'));
