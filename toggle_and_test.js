...
async function main() {
  ...
  console.log('Deactivate:', deact.s, deact.b.name);
  
  // Activate
  console.log('Activating...');
  const act = await api('POST', 'workflows/WFCRM001chat01/activate');
  console.log('Activate:', act.s, act.b.name);
  
  // Test
  console.log('Testing webhook...');
  const body = 'user_id=41fcf46d-e02a-439e-93af-bfc8ae614809&name=Kevin&message=teste%20buffer';
  const test = await api('POST', 'webhook/crm-chat', body);
  console.log('Test:', test.s, JSON.stringify(test.b).substring(0, 300));
}
