...
export default function AdminPage() {
  ...
  async function handleToggleActive(user: UserItem) {
    ...
    try {
      ...
      if (!res.ok) setUsers((prev) => prev.map((u) => (u.id === user.id ? { ...u, active: user.active } : u)))
    } catch {
  ...
  async function handleResetPassword(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await res.json()
      if (res.ok) {
        setResetMessage({ type: "success", text: `Senha de ${selectedUser.name || selectedUser.email} atualizada!` })
    ...
    <div className="min-h-screen bg-[#131520] p-4 md:p-6 lg:p-8 font-sans">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <PageHeader
          ...
          description="Gerencie acesso ao sistema e senhas dos usuários"
          actions={
            <button
              onClick={loadUsers}
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="rounded-2xl border border-[#1e2030] bg-[#1f2136] p-6 shadow-xl space-y-4">
              ...
              {loadingUsers && users.length === 0 ? (
                <LoadingState variant="skeleton" rows={4} message="Carregando usuários..." />
              ) : users.length === 0 ? (
                <EmptyState
                  title="Nenhum usuário encontrado"
                  description="Crie o primeiro usuário ao lado ou sincronize com funcionários."
                />
              ) : (
                <div className="divide-y divide-[#2a2d45] overflow-hidden rounded-xl border border-[#2a2d45]">
                  {users.map((u) => {
                    ...
                    return (
                      <div
                        key={u.id}
                        className={`flex flex-col sm:flex-row sm:items-center justify-between p-4 gap-4 transition-colors ${isActive ? "bg-[#171928] hover:bg-[#1c1e30]" : "bg-[#141522] opacity-75"}`}
                      >
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                          <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full font-bold text-sm ${isActive ? "bg-indigo-500/20 text-indigo-400 border border-indigo-500/30" : "bg-slate-700/30 text-slate-500 border border-slate-700/50"}`}>
          ...
          <div className="space-y-6">
            <div className="rounded-2xl border border-[#1e2030] bg-[#1f2136] p-6 shadow-xl space-y-4">
              ...
              <form onSubmit={handleCreate} className="space-y-3">
                ...
                <div>
                  <label className="text-xs font-medium text-slate-300 block mb-1">Senha <span className="text-slate-500">(opcional)</span></label>
                  <input
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="w-full max-w-md rounded-2xl border border-[#2a2d45] bg-[#1f2136] p-6 shadow-2xl space-y-5 relative">
            ...
            <form onSubmit={handleResetPassword} className="space-y-4">
              ...
              {resetMessage && (
                <div className={`flex items-center gap-2 text-xs rounded-xl p-3 border ${resetMessage.type === "success" ? "text-emerald-400 bg-emerald-500/10 border-emerald-500/30" : "text-rose-400 bg-rose-500/10 border-rose-500/30"}`}>
              ...
              <div className="flex items-center gap-3 pt-2">
                <button type="submit" disabled={resettingPwd} className="flex-1 inline-flex h-11 items-center justify-center rounded-xl bg-indigo-600 hover:bg-indigo-500 text-xs font-bold text-white shadow-md transition-all disabled:opacity-60">
                  {resettingPwd ? "Atualizando..." : "Salvar Nova Senha"}
                </button>
                <button type="button" onClick={() => setSelectedUser(null)} className="h-11 px-4 rounded-xl border border-[#2a2d45] bg-[#131520] text-xs font-medium text-slate-300 hover:text-white hover:bg-[#2a2d45] transition-all">
                  Cancelar
                </button>
              </div>
            </form>
